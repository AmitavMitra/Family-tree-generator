...
try:
    ancestry = pd.read_csv('sample.csv')
except Exception as e:
    print("File not loaded")
print("File loaded")
earl_ans = ancestry.loc[ancestry['Relation'] == 'First_ancestor', 'Person1'].iloc[0]
...
det = str(ancestry.loc[ancestry['Person1'] == earl_ans, 'Details'][0])
g = ancestry.loc[ancestry['Person1'] == earl_ans, 'Gender'][0]
sh = 'rect' if g == 'M' else 'ellipse'
dot.node(earl_ans, earl_ans, tooltip = det, shape = sh)
node_nm.append(earl_ans)

ancestry.loc[ancestry['Person1'] == earl_ans, 'recorded_ind'] = 1
    ...
    if len(temp) == 0:      # Break loop when all individuals have been recorded
        ...
        if len(this_gen) > 0:
            for j in range(0, len(this_gen)):
                per1 = this_gen['Person1'].iloc[j]
                per2 = this_gen['Person2'].iloc[j]
        ...
        comp.extend(incomp)
        incomp = list(temp.loc[temp['this_gen_ind'] == 1, 'Person1'])
        ancestry['recorded_ind'] = temp.apply(lambda x: 1 if (x['Person1'] in incomp) | (x['Person1'] in comp) else 0, axis = 1)

dot.format = 'svg'
dot.render('family_tree.gv.svg', view = True)